...
pub fn error_reporting_unary_operator(err_message: String, val1: &TypeVal) -> Result<TypeVal, String> {
    let err_mess = err_message.red();
...
pub fn error_reporting_binary_operator(err_message: String, val1: &TypeVal, val2: &TypeVal) -> Result<TypeVal, String> {
    let err_mess = err_message.red();
    let var1 = format!("{:?}", val1);
